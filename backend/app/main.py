...
from backend.app.services.rag_service import SimpleRAGService
...
app = FastAPI(title=f"{domain.manifest.branding.app_name} Workshop")
app.add_middleware(
    CORSMiddleware,
...
_MEMORY_TOOL_NAMES = {"search_customer_memory", "remember_customer_detail"}
...
# ── Workshop status endpoint ──


@app.get("/api/status")
async def get_status() -> JSONResponse:
    """Return which Iris modules are configured and working."""
    return JSONResponse({
        "rag_enabled": rag_service.is_configured(),
        "guardrail_enabled": guardrail_service.is_configured(),
        "langcache_enabled": langcache_service.is_configured(),
        "context_surfaces_enabled": cs_service.is_configured(),
    ...
    if memory_service.is_configured() and final_text.strip():
        try:
            await memory_service.add_session_event(
                ...
                metadata={"source": "workshop", "mode": request.mode},
            )
        except Exception as exc:
