...
class HealthcareDomain:
    manifest = DomainManifest(
        ...
        branding=BrandingConfig(
            ...
            starter_prompts=[
                PromptCard(eyebrow="Context", title="Do I have any upcoming appointments?", prompt="Do I have any upcoming appointments?"),
    ...
    def write_dataset_meta(self, *, settings: Any, records: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
        summary = {
            "locations": len(records.get("Location", [])),
            "providers": len(records.get("Provider", [])),
            "patients": len(records.get("Patient", [])),
            "appointments": len(records.get("Appointment", [])),
            "referrals": len(records.get("Referral", [])),
            "waitlist": len(records.get("Waitlist", [])),
            "healthdocs": len(records.get("HealthDoc", [])),
        }
        client = create_redis_client(settings)
