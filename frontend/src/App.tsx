...
export default function App() {
  ...
  const [mode, setMode] = useState<AgentMode>(
    () => (localStorage.getItem(modeStorageKey) as AgentMode) || "simple_rag"
  );
  ...
  useEffect(() => {
    document.title = domain?.app_name ?? "Redis Iris Workshop";
  }, [domain]);
  ...
  const visiblePrompts = allQuickStarts.filter((p) => {
    if (p.eyebrow === "Memory" && !status?.memory_enabled) return false;
    if (p.eyebrow === "Cached" && !status?.langcache_enabled) return false;
    return true;
  });
