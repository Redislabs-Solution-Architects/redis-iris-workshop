...
def _update_surface(
    *,
    ...
    if response.status_code not in (200, 201):
        raise RuntimeError(
            "Failed to update context surface "
            f"(status {response.status_code}): {_safe_response_text(response)}"
        )
    return response.json()
...
def main() -> None:
    ...
    if surface_id:
        print(f"Found existing context surface: {surface_id}")
        try:
            ...
            sys.exit(1)
        print("Updating surface with full data model (9 entities)...")
        _update_surface(
            api_url=api_url,
